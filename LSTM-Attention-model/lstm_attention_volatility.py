...
class VolatilityLSTM(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Context vector is the weighted sum of all hidden states across time
...
def run(args: argparse.Namespace) -> None:
    ...
    pd.DataFrame(selection_rows).to_csv(OUT_DIR / "lstm_attention_validation_results.csv", index=False)
    pd.DataFrame(test_rows).to_csv(OUT_DIR / "lstm_attention_test_results.csv", index=False)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--lookbacks", default="5,10,22,66", help="Comma-separated sequence lengths.")
    parser.add_argument("--hidden-sizes", default="16,32,64", help="Comma-separated hidden sizes.")
