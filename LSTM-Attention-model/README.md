...
`lstm_attention_volatility.py` implements the project-plan LSTM approach on the prepared
`data/splits` grid:
...
```powershell
python .\LSTM-Attention-model\lstm_attention_volatility.py --targets SPY --freqs daily --exogs with_exog --epochs 20 --lookbacks 22 --hidden-sizes 32 --dropouts 0.0
```

## Run the full 12-cell grid

```powershell
python .\LSTM-Attention-model\lstm_attention_volatility.py
```

Outputs are written to `LSTM-Attention-model/outputs/`:
...
```powershell
python .\LSTM-Attention-model\lstm_attention_volatility.py --no-plots
```

## RFP (Random Forecast Periods) evaluation

`lstm_attention_rfp.py` evaluates the validated LSTM blueprints on the RFP windows
...
```bash
python LSTM-Attention-model/lstm_attention_rfp.py --targets SPY --freqs daily --exogs no_exog --regimes GFC --cpu
```

### Run all 12 cells across all regimes

```bash
python LSTM-Attention-model/lstm_attention_rfp.py
```
...
Written to `LSTM-Attention-model/outputs/rfp/`:
