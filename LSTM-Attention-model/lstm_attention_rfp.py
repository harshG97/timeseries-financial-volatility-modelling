...
Reads the selected LSTM hyperparameters from ``lstm_attention_validation_results.csv``
...
Outputs
-------
LSTM-Attention-model/outputs/rfp/
    lstm_attention_rfp_results.csv        -- one row per (cell × window)
    lstm_attention_rfp_summary.csv        -- per-cell and per-regime aggregates
...
_lstm_path = Path(__file__).resolve().parent / "lstm_attention_volatility.py"
_spec = spec_from_file_location("lstm_attention_volatility", _lstm_path)
_lstm_mod = module_from_spec(_spec)
sys.modules["lstm_attention_volatility"] = _lstm_mod  # must register before exec for @dataclass
...
def run(args: argparse.Namespace) -> None:
    ...
    if not val_csv.exists():
        raise FileNotFoundError(
            ...
            "Run lstm_attention_volatility.py first to generate it."
        )
    ...
    results_df.to_csv(RFP_OUT / "lstm_attention_rfp_results.csv", index=False)
    ...
    summary.to_csv(RFP_OUT / "lstm_attention_rfp_summary.csv", index=False)

    print(f"\n{'='*60}")
    print(f"Results saved to {RFP_OUT.relative_to(ROOT)}/")
    print(f"  lstm_attention_rfp_results.csv   ({len(results_df)} rows)")
    print(f"  lstm_attention_rfp_summary.csv   ({len(summary)} rows)")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        "--validation-csv",
        default="lstm_attention_validation_results.csv",
        help="Path to validation results CSV (default: outputs/lstm_attention_validation_results.csv)",
    )
