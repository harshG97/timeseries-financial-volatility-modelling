"""
Transformer RFP (Random Forecast Periods) evaluation.

Reads the selected Transformer hyperparameters from ``transformer_validation_results.csv``
(one blueprint per cell), then evaluates each blueprint on every RFP window
using ``src/rfp_generator.py``.

Outputs
-------
additional-models/outputs/transformer/rfp/
    transformer_rfp_results.csv
    transformer_rfp_summary.csv
    forecasts/
    plots/
"""
...
# Re-use building blocks from the main Transformer script
from importlib.util import spec_from_file_location, module_from_spec

_transformer_path = Path(__file__).resolve().parent / "transformer_volatility.py"
_spec = spec_from_file_location("transformer_volatility", _transformer_path)
_mod = module_from_spec(_spec)
sys.modules["transformer_volatility"] = _mod  # must register before exec for @dataclass
_spec.loader.exec_module(_mod)

TransformerConfig = _mod.TransformerConfig
VolatilityTransformer = _mod.VolatilityTransformer
...
make_sequences = _mod.make_sequences
fit_scaler = _mod.fit_scaler
...
def load_validation_configs(csv_path: Path) -> dict[tuple[str, str, str], TransformerConfig]:
    """Load per-cell best configs from the validation results CSV."""
    df = pd.read_csv(csv_path)
    configs: dict[tuple[str, str, str], TransformerConfig] = {}
    for _, row in df.iterrows():
        key = (row["target"], row["freq"], row["exog"])
        configs[key] = TransformerConfig(
...
def evaluate_window(
    ...
    """Train Transformer on a single RFP window and return (metrics_dict, forecast_df)."""
...
def run(args: argparse.Namespace) -> None:
    ...
    if not val_csv.exists():
        raise FileNotFoundError(
            ...
            "Run transformer_volatility.py first to generate it."
        )
    ...
    results_df.to_csv(RFP_OUT / "transformer_rfp_results.csv", index=False)
    ...
    summary.to_csv(RFP_OUT / "transformer_rfp_summary.csv", index=False)

    print(f"\n{'='*60}")
    print(f"Results saved to {RFP_OUT.relative_to(ROOT)}/")
    print(f"  transformer_rfp_results.csv   ({len(results_df)} rows)")
    print(f"  transformer_rfp_summary.csv   ({len(summary)} rows)")
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description="Evaluate Transformer volatility models on RFP windows."
    )
    p.add_argument(
        "--targets", default="all",
    ...
    p.add_argument(
        "--validation-csv",
        default="transformer_validation_results.csv",
        help="Path to validation results CSV (default: outputs/transformer/transformer_validation_results.csv)",
    )
