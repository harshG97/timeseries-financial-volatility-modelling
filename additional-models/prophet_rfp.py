"""
Prophet RFP (Random Forecast Periods) evaluation.

Reads the selected Prophet hyperparameters from ``prophet_validation_results.csv``
(one blueprint per cell), then evaluates each blueprint on every RFP window
using ``src/rfp_generator.py``.

Outputs
-------
additional-models/outputs/prophet/rfp/
    prophet_rfp_results.csv
    prophet_rfp_summary.csv
    forecasts/
    plots/
"""
...
# Re-use building blocks from the main Prophet script
from importlib.util import spec_from_file_location, module_from_spec

_path = Path(__file__).resolve().parent / "prophet_volatility.py"
_spec = spec_from_file_location("prophet_volatility", _path)
_mod = module_from_spec(_spec)
sys.modules["prophet_volatility"] = _mod  # must register before exec for @dataclass
_spec.loader.exec_module(_mod)

ProphetConfig = _mod.ProphetConfig
fit_prophet = _mod.fit_prophet
...
def load_validation_configs(csv_path: Path) -> dict[tuple[str, str, str], ProphetConfig]:
    df = pd.read_csv(csv_path)
    configs: dict[tuple[str, str, str], ProphetConfig] = {}
    for _, row in df.iterrows():
        key = (row["target"], row["freq"], row["exog"])
        configs[key] = ProphetConfig(
            ...
            yearly_seasonality=row["yearly_seasonality"],
...
def evaluate_window(
    ...
    """Train Prophet on a single RFP window and return (metrics_dict, forecast_df)."""
    train_df = window.train
    forecast_df = window.forecast
    columns = feature_columns(train_df)

    model = fit_prophet(train_df, columns, config, window.freq)

    # Batch predict
    future = pd.DataFrame({"ds": pd.to_datetime(forecast_df["date"]).values})
    for col in columns:
        future[col] = forecast_df[col].to_numpy(dtype=np.float64)
    
    pred = model.predict(future)
    yhat_log = pred["yhat"].values
...
def run(args: argparse.Namespace) -> None:
    ...
    if not val_csv.exists():
        raise FileNotFoundError(
            ...
            "Run prophet_volatility.py first to generate it."
        )
    ...
    results_df.to_csv(RFP_OUT / "prophet_rfp_results.csv", index=False)
    ...
    summary.to_csv(RFP_OUT / "prophet_rfp_summary.csv", index=False)

    print(f"\n{'='*60}")
    print(f"Results saved to {RFP_OUT.relative_to(ROOT)}/")
    print(f"  prophet_rfp_results.csv   ({len(results_df)} rows)")
    print(f"  prophet_rfp_summary.csv   ({len(summary)} rows)")
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description="Evaluate Prophet volatility models on RFP windows."
    )
    p.add_argument(
        "--targets", default="all",
