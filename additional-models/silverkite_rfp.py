"""
SilverKite RFP (Random Forecast Periods) evaluation.

Reads the selected SilverKite hyperparameters from ``silverkite_validation_results.csv``
(one blueprint per cell), then evaluates each blueprint on every RFP window
using ``src/rfp_generator.py``.

Outputs
-------
additional-models/outputs/silverkite/rfp/
    silverkite_rfp_results.csv
    silverkite_rfp_summary.csv
    forecasts/
    plots/
"""
...
def set_seed(seed: int) -> None:
    ...
    Note: Greykite's SimpleSilverkiteEstimator does not expose a seed
    argument and uses internal randomness for components like changepoint
    detection. Full bit-identical reproducibility is not guaranteed; this
    pins everything we control.
    """
...
# Re-use building blocks from the main SilverKite script
from importlib.util import spec_from_file_location, module_from_spec

_path = Path(__file__).resolve().parent / "silverkite_volatility.py"
_spec = spec_from_file_location("silverkite_volatility", _path)
_mod = module_from_spec(_spec)
sys.modules["silverkite_volatility"] = _mod  # must register before exec for @dataclass
_spec.loader.exec_module(_mod)

SilverKiteConfig = _mod.SilverKiteConfig
fit_silverkite = _mod.fit_silverkite
...
def load_validation_configs(csv_path: Path) -> dict[tuple[str, str, str], SilverKiteConfig]:
    df = pd.read_csv(csv_path)
    configs: dict[tuple[str, str, str], SilverKiteConfig] = {}
    for _, row in df.iterrows():
        key = (row["target"], row["freq"], row["exog"])
        configs[key] = SilverKiteConfig(
            fit_algorithm=row["fit_algorithm"],
            yearly_seasonality=int(row["yearly_seasonality"]),
            weekly_seasonality=int(row["weekly_seasonality"]),
            growth_term=row["growth_term"],
            changepoints_method=row["changepoints_method"] if pd.notna(row["changepoints_method"]) else "",
            use_regressors=bool(row["use_regressors"]),
            feature_sets_enabled=bool(row["feature_sets_enabled"]),
        )
    return configs
...
def evaluate_window(
    ...
    """Train SilverKite on a single RFP window and return (metrics_dict, forecast_df)."""
    train_df = window.train
    forecast_df = window.forecast
    columns = feature_columns(train_df)

    model = fit_silverkite(train_df, columns, config, window.freq)

    # Batch predict
    future = pd.DataFrame({"ts": pd.to_datetime(forecast_df["date"]).values})
    for col in columns:
    ...
    yhat_log = pred_df[yhat_col].values
...
def run(args: argparse.Namespace) -> None:
    ...
    if not val_csv.exists():
        raise FileNotFoundError(
            ...
            "Run silverkite_volatility.py first to generate it."
        )
    ...
    for target, freq, exog in tqdm(cells, desc="RFP cells"):
        ...
        for w in tqdm(windows, desc=f"  windows {target}/{freq}/{exog}", leave=False):
            m, fc_frame = evaluate_window(w, config)
    ...
    results_df.to_csv(RFP_OUT / "silverkite_rfp_results.csv", index=False)
    ...
    summary.to_csv(RFP_OUT / "silverkite_rfp_summary.csv", index=False)

    print(f"\n{'='*60}")
    print(f"Results saved to {RFP_OUT.relative_to(ROOT)}/")
    print(f"  silverkite_rfp_results.csv   ({len(results_df)} rows)")
    print(f"  silverkite_rfp_summary.csv   ({len(summary)} rows)")
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description="Evaluate SilverKite volatility models on RFP windows."
    )
    p.add_argument(
        "--targets", default="all",
    ...
    p.add_argument(
        "--validation-csv",
        default="silverkite_validation_results.csv",
        help="Path to validation results CSV (default: outputs/silverkite/silverkite_validation_results.csv)",
    )
    p.add_argument("--seed", type=int, default=42,
                   help="Seed for Python/NumPy RNGs (Greykite internals are "
